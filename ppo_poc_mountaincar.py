...
class PPO():
    def __init__(self, actor=None, critic=None, learning_rate=1e-3, env_name="CartPole-v1",
        ...
        self.normalized_returns = Deque(maxlen=10000)
        self.normalized_returns.append(-1)
        self.normalized_returns.append(-1*.9+-1)
    ...
    def learn(self):
        ...
        episodic_reward = 0
        ret = 0
        max0, max1 = -np.inf, -np.inf
        min0, min1 = np.inf, np.inf
        while total_timesteps < self.N_TIMESTEPS:
            ...
            while rollout_timesteps < self.N_ROLLOUT_TIMESTEPS:
                with torch.no_grad():
                    state = _state
                    # max0 = max(state[0],max0)
                    # min0 = min(state[0], min0)
                    # max1 = max(state[1],max1)
                    # min1 = min(state[1], min1)
